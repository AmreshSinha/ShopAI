        ...
        <div>
            {
                respones.map((queryResult, idx) => {
                    ...
                    if ("recommendations" in queryResult.response) {
                        return <RecommendationTile recommendationsList={queryResult.response.recommendations} />
                    }
                    else return <QuestionTile question={queryResult.response.question}/>
                })
            }
        </div>
        {
        ...
        <div className="p-6 flex justify-center">
            <div className="chatbar-wrapper" >
                <form onSubmit={handleSubmit}>
                    ...
                    <input type="text" className="text-field" autoFocus value={askedQuery} onInput={e => setAskedQuery(e.target.value)} />
                </form>
            </div>
        </div>
    </div>);
